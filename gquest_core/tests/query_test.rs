...
fn parse_query() {
    ...
    assert!(
        matches!(QueryParser::parse_query("x = 1"), Ok(ParsedQuery::Condition(x)) if x == sql_cond )
    );

    assert!(
        matches!(QueryParser::parse_query("min(p_gn: g, d), x = 1"), Ok(ParsedQuery::Extremal(val)) if val == extremal )
    );

    assert!(
        matches!(QueryParser::parse_query("min(p_gn: g, d), x = 1 => !(1 = p)"), Ok(ParsedQuery::ExtremalCounter(val)) if val == conjecture )
    )
}
...
fn parse_expression_comparison() {
    assert!(matches!(
        QueryParser::parse_condition("n + 1 > 0"),
    ...
    assert!(matches!(
        QueryParser::parse_condition("floor(n) = 0 % (2 * 1)"),
        Ok(SqlCondition::Operation(SqlComparison::Equal(a, b)))
        if
    ...
    assert!(matches!(
        QueryParser::parse_condition("a = n ** 2"),
        Ok(SqlCondition::Operation(SqlComparison::Equal(a, b)))
        if
